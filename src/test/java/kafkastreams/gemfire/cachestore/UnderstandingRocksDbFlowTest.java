...
public class UnderstandingRocksDbFlowTest {
    ...
    private List<KeyValue<String, String>> data5 =
            Arrays.asList(
                    new KeyValue<>(
                            "key1",
                            "Should_get_joined_Key1_again"));
    ...
    public void howDoesRocksDbWorkTest() throws ExecutionException, InterruptedException {
        ...
        IntegrationTestUtils.produceKeyValuesSynchronously(
                updateTopic, data5, messageProducerConfig, Time.SYSTEM);
        List<KeyValue<String, String>> valueAfterReceivingData = IntegrationTestUtils.waitUntilMinKeyValueRecordsReceived(messageConsumerConfig, latestTopic, 11);
        streams.close();
        valueAfterReceivingData.stream().forEach(System.out::println);
    }

    public void joinStreams(StreamsBuilder kStreamBuilder, String latestTopic) {
...
//        StoreBuilder<KeyValueStore<String, String>> storeBuilder = Stores.keyValueStoreBuilder(Stores.persistentKeyValueStore("test-store"),Serdes.String(),Serdes.String()).withLoggingDisabled();
        KTable<String,String> latestTable = kStreamBuilder.table(latestTopic, Materialized.<String,String>as(Stores.persistentKeyValueStore("test-store")).withLoggingDisabled());

        KStream<String,String> updateStream = kStreamBuilder.stream(updateTopic);

        updateStream.join(latestTable, (left,right) -> right+" + "+left).to(latestTopic);
